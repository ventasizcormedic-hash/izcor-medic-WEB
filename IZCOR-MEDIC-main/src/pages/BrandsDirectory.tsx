...
export function BrandsDirectory() {
  ...
  const fetchBrands = async (searchStr = '', pageNum = 1) => {
    setLoading(true);
    try {
  ...
  return (
    <>
      <SeoHead
        title="Directorio de Marcas Médicas | IZCOR MEDIC"
        description="Directorio completo de marcas de equipos e insumos médicos disponibles en Izcor Medic. Encuentra productos de las mejores marcas del sector salud."
        canonicalUrl="/marcas"
      ...
      <div className="min-h-screen bg-[#F6F8FC] py-8 px-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex text-xs font-semibold text-slate-500" aria-label="Breadcrumb">
          <ol className="inline-flex items-center space-x-1.5 md:space-x-2">
            ...
            <li className="text-slate-900 font-bold">Marcas Comerciales</li>
          </ol>
        </nav>
        ...
        <div className="bg-white rounded-3xl border border-slate-200/90 p-8 shadow-xs relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-3xl">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md text-[11px] font-extrabold uppercase tracking-wider bg-[#2C3E50] text-white mb-4 shadow-xs">
              <ShieldCheck className="w-3.5 h-3.5 text-cyan-300" />
              Directorio de Marcas Autorizadas
            </div>
            <h1 className="text-3xl lg:text-4xl font-black text-[#2C3E50] tracking-tight mb-3">
              Directorio de Marcas Comerciales
            </h1>
            <p className="text-slate-600 text-sm md:text-base leading-relaxed mb-6">
              Consulte el listado oficial de marcas de equipamiento médico disponibles en el catálogo. Cada marca muestra su fabricante respectivo y el volumen de productos verificados.
            </p>
            ...
            <div className="flex items-center gap-2 border-b border-slate-200 pb-4">
              ...
              <Link 
                to="/marcas"
                className="px-4 py-2 rounded-xl bg-[#2C3E50] text-white font-bold text-xs shadow-2xs"
              >
                Marcas Comerciales
              </Link>
            </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          <form onSubmit={handleSearchSubmit} className="relative flex-1">
            ...
            <input
              ...
              placeholder="Buscar por marca, fabricante o descripción..."
              className="w-full pl-10 pr-10 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:border-brand-cyan focus:ring-2 focus:ring-brand-cyan/10 bg-slate-50/50 focus:bg-white text-slate-900 transition-all"
            />
          </form>
          ...
          <div className="flex items-center gap-3 text-xs text-slate-500 font-semibold px-2 flex-shrink-0">
            <span className="flex items-center gap-1.5">
              ...
              <span>{total} Marcas Registradas</span>
            </span>
          </div>
        </div>
        ...
        {loading ? (
          <div className="py-24 text-center">
            ...
            <p className="text-sm font-bold text-[#2C3E50]">Cargando directorio de marcas...</p>
          </div>
        ) : brands.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {brands.map((brand) => (
              <div 
                ...
                className="bg-white rounded-2xl border border-slate-200/90 p-6 shadow-xs hover:shadow-md hover:border-brand-cyan/30 transition-all cursor-pointer group flex flex-col justify-between"
              >
                <div>
