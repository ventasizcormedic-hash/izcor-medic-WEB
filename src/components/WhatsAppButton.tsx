...
export function WhatsAppButton() {
  ...
  const getWhatsAppMessage = () => {
    ...
    if (path.startsWith('/productos/')) {
      return 'Hola IZCOR MEDIC, me encuentro revisando este producto en su catálogo web y deseo consultar disponibilidad, ficha técnica y cotización formal.';
    }
    if (path === '/catalogo') {
      return 'Hola IZCOR MEDIC, deseo solicitar asesoría médica y cotización de equipos e insumos del catálogo.';
    }
    if (path === '/cotizar' || path === '/cotizacion') {
      return 'Hola IZCOR MEDIC, tengo una consulta sobre mi solicitud de cotización formal de equipamiento médico.';
    }
    if (path === '/tdr') {
      return 'Hola IZCOR MEDIC, deseo coordinar la revisión técnica de Términos de Referencia (TDR) y homologación para licitación o compra institucional.';
    }
    if (path === '/contacto') {
      return 'Hola IZCOR MEDIC, deseo comunicarme directamente con un asesor comercial de equipamiento médico.';
    }
    return 'Hola IZCOR MEDIC, solicito información, disponibilidad y cotización de equipos e insumos médicos.';
  };

  const whatsappUrl = `https://wa.me/51928130349?text=${encodeURIComponent(getWhatsAppMessage())}`;
    ...
    <div className="fixed bottom-[max(1.5rem,env(safe-area-inset-bottom))] right-[max(1rem,env(safe-area-inset-right))] z-50 flex items-center gap-4 sm:right-6">
      <AnimatePresence>
        {showTooltip && (
          ...
          >
            <p className="text-sm font-bold text-brand-navy mb-0.5">¿Cotización Rápida?</p>
            <p className="text-xs text-slate-500">Atención médica directa por WhatsApp</p>
            {/* Tooltip triangle */}
