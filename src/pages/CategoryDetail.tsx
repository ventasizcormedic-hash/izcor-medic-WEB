  ...
  const categorySchema = [
    {
      ...
      "@type": "CollectionPage",
      "name": category.name,
      "description": category.description || `Equipamiento médico y productos especializados en ${category.name}.`,
      "url": typeof window !== 'undefined' ? `${window.location.origin}/categorias/${category.slug}` : undefined,
    },
    {
      "@context": "https://schema.org/",
      "@type": "BreadcrumbList",
      "itemListElement": [
        ...
        parentCategory ? {
          ...
          "item": typeof window !== 'undefined' ? `${window.location.origin}/categorias/${parentCategory.slug}` : ""
        } : null,
        {
          "@type": "ListItem",
          "position": parentCategory ? 4 : 3,
          "name": category.name,
          "item": typeof window !== 'undefined' ? `${window.location.origin}/categorias/${category.slug}` : ""
        }
      ].filter(Boolean)
    }
  ];
